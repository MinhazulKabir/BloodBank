            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    @if ($blog->blog_picture)
                        <a href="{{ asset(env('UPLOAD_PATH').'/'.$blog->blog_picture) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/'.$blog->blog_picture) }}"></a>
                    @endif
                    {!! Form::label('blog_picture', trans('quickadmin.blog.fields.blog-picture').'*', ['class' => 'control-label']) !!}
                    {!! Form::file('blog_picture', ['class' => 'form-control', 'style' => 'margin-top: 4px;']) !!}
